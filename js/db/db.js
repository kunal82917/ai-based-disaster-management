...
const SCHEMA = {
  reports: {
    keyPath: "id",
    indexes: [
      { name: "bySender", key: "sender" },
      { name: "byApproved", key: "approved" },
      { name: "byType", key: "type" }
    ]
  },
  alerts: {
    keyPath: "id",
    indexes: [{ name: "byTimestamp", key: "timestamp" }]
  },
  analysis: { keyPath: "id" },
  news: { keyPath: "id" },
  messages: { keyPath: "id" }
...
export function getAll(storeName, callback) {
  try {
...
export function addItem(storeName, data, onSuccess, onError) {
  try {
...
export function updateItem(storeName, data, onSuccess) {
  try {
    const tx = getDB().transaction(storeName, "readwrite");
...
export function deleteItem(storeName, id, onSuccess) {
  try {
    const tx = getDB().transaction(storeName, "readwrite");
...
let contactSettings =
  JSON.parse(localStorage.getItem("ocean_contact")) || {
